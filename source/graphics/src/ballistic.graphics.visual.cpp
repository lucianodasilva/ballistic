...
namespace ballistic {
	namespace graphics {
		...
		const id_t visual::component_id = ballistic::id::visual::id;

		void visual::require_properties (entity_type * new_type, component_info & info) {
			
			new_type->properties
				.require < id_t > (id::visual::material_id, id::null)
				.require < id_t > (id::visual::mesh_id, id::null)
				.require < id_t > (id::visual::rig_id, id::null)
				.require < material * > (id::visual::material, nullptr)
				.require < imesh * > (id::visual::mesh, nullptr)
				.require < rig_frame_tween * > (id::visual::rig_tween, &rig_frame_tween::null_frame_tween)
		...
		void visual::setup (ballistic::containers::property_container & parameters)
		{
			...
			if (!*_material)
				*_material = g.resources [
					p.properties [id::visual::material_id].as < id_t >()
				].as < material > () ;

			if (!*_mesh)
				*_mesh = g.resources [
					p.properties [id::visual::mesh_id].as < id_t > ()
				].as < imesh > ();

			_rig_tween = p.properties.aquire < rig_frame_tween * > (id::visual::rig_tween);
